...
import { useGlobalContext2 } from "../../../UnContexte";

export default function Verification(eventId) {
  const { eventid, setEventId } = useGlobalContext2();
        ...
        if (result.toString().split(" ")[0] !== "unknown") {
          ...
          eventUsers.forEach((user) => {
            if (user.user.id === result.toString().split(" ")[0]) {
              ...
              drawBox.draw(canvas);
              const nameh1 = document.getElementById("nameh1");
              nameh1.innerHTML = user.user.username + " " + "☑ Verified";
